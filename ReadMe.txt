========================================================================
       MICROSOFT FOUNDATION CLASS ライブラリ : DigitShowBasic
========================================================================


AppWizard が生成した DigitShowBasic には、Microsoft Foundation Class の基本的な
使い方が示されています。アプリケーション作成のひな型としてお使いください。

このファイルには DigitShowBasic を構成している各ファイルの概要説明が含まれています。
...
DigitShowBasic.h
    ...
    みます） をインクルードし、また CDigitShowBasicApp アプリケーションクラスを宣
    言します。

DigitShowBasic.cpp
    このファイルは CDigitShowBasicApp アプリケーションクラスを含むアプリケーション
    の中心となるソースファイルです。
...
res\DigitShowBasic.ico
    このファイルは、アプリケーションのアイコンとして使用されるアイコンファイ
    ルです。このアイコンはリソース ファイル DigitShowBasic.rc によってインクルード
    されます。

res\DigitShowBasic.rc2
...
DigitShowBasicDoc.h, DigitShowBasicDoc.cpp - ドキュメント
    これらのファイルは CDigitShowBasicDoc クラスを含みます。特別なドキュメント デ
    ータを付け加えたり、CDigitShowBasicDoc::Serialize を使ったファイルのセーブや
    ロード機能をインプリメントしたいときはこのファイルを編集してください。

DigitShowBasicView.h, DigitShowBasicView.cpp - ビュー
    これらのファイルは CDigitShowBasicView クラスを含みます。
    CDigitShowBasicView オブジェクトは CDigitShowBasicDoc オブジェクトを表示するために
    使います。
...
StdAfx.h, StdAfx.cpp
    これらのファイルはプリコンパイル済ヘッダー ファイル (PCH) DigitShowBasic.pch 
